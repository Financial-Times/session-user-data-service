...
var ArticleDataStore = function (articleId) {
	...
	function fetchCommentCount (callback) {
		livefyreService.getCommentCount(articleId, function (err, data) {
			if (err) {
				callback(err);
				return;
			}
	...
	function upsertCommentCount (count) {
		upsertStoredData("commentCount", {
			count: count,
			expires: new Date(new Date().getTime() + 1000 * 60 * 5)
		});
	}
	this.getCommentCount = function (callback) {
		if (typeof callback !== 'function') {
			throw new Error("ArticleDataStore.getLivefyreCollectionDetails: callback not provided");
		}
		...
		getStoredData(function (errCache, storedData) {
			if (errCache) {
				// fetch
				consoleLogger.log(articleId, 'commentCount', 'cache down');

				fetchCommentCount(function (errFetch, count) {
					if (errFetch) {
						if (errFetch.unclassified === true || errFetch.statusCode === 404) {
							callback(null, 0);
							return;
						}
			...
			if (storedData && storedData.commentCount && typeof storedData.commentCount.count !== 'undefined') {
				consoleLogger.log(articleId, 'commentCount', 'loaded from cache');

				if (new Date(storedData.commentCount.expires) < new Date()) {
					// expired, fetch and update
					consoleLogger.log(articleId, 'commentCount', 'cache expired');
					fetchCommentCount(function (errFetch, count) {
						if (errFetch) {
							if (errFetch.unclassified === true || errFetch.statusCode === 404) {
								callback(null, 0);
						...
						callback(null, count);
						upsertCommentCount(count);
					});
				} else {
					callback(null, storedData.commentCount.count);
				}
			} else {
				// fetch and save
				consoleLogger.log(articleId, 'commentCount', 'not found in cache');
				fetchCommentCount(function (errFetch, count) {
					if (errFetch) {
						if (errFetch.unclassified === true || errFetch.statusCode === 404) {
							callback(null, 0);

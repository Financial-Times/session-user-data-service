...
function expectProtected (method, endpoint) {
	describe(`\`${method} ${endpoint}\``, function () {
		...
		it('is accepted if there is a valid API key', function (done) {
			makeRequest()
				.set('X-Api-Key', process.env.API_KEY_FOR_RESTRICTED_ENDPOINTS)
				.set('Origin', 'https://www.ft.com')
				.expect(200, done);
		});

		it('is accepted if there is a valid API key but a bad Origin', function (done) {
			makeRequest()
				.set('X-Api-Key', process.env.API_KEY_FOR_RESTRICTED_ENDPOINTS)
				.set('Origin', 'https://www.notft.com')
				.expect(200, done);
		...
		it('is accepted if there is a valid API key but a bad Referrer', function (done) {
			makeRequest()
				.set('X-Api-Key', process.env.API_KEY_FOR_RESTRICTED_ENDPOINTS)
				.set('Referer', 'https://www.notft.com')
				.expect(200, done);
